...
void mainsystem::fir8_main()
{
	...
	{HEPSY_S(fir8m_id)
		...
		HEPSY_S(fir8m_id) fir8e_parameters_channel_1->write(fir8e_p);
		HEPSY_S(fir8m_id) fir8e_r=fir8e_results_channel_2->read();
		...
		// send parameters and receive results
		HEPSY_S(fir8m_id) fir8s_parameters_channel_1->write(fir8s_p);
		HEPSY_S(fir8m_id) fir8s_r=fir8s_results_channel_2->read();

		// fill local variables
		HEPSY_S(fir8m_id) for( unsigned j=0; j<TAP8; j++) shift[j]=fir8s_r.shift[j];
		
		// write output values on channel
		HEPSY_S(fir8m_id) result8_channel_1->write(acc);
