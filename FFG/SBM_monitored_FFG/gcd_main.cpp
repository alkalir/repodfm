...
void mainsystem::gcd_main()
{
	...
	{HEPSY_S(gcdm_id)
		...
		HEPSY_S(gcdm_id) sample1=result8_channel_2->read();
		HEPSY_S(gcdm_id) if (sample1==0) err=true;

		HEPSY_S(gcdm_id) sample2=result16_channel_2->read();
		...
		{HEPSY_S(gcdm_id)  
			...
			HEPSY_S(gcdm_id) gcde_parameters_channel_1->write(gcde_p);
			HEPSY_S(gcdm_id) gcde_r=gcde_results_channel_2->read();
