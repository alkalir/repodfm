...
void mainsystem::fir16_main()
{
	...
	{HEPSY_S(fir16m_id)
		...
		HEPSY_S(fir16m_id) fir16e_parameters_channel_1->write(fir16e_p);
		HEPSY_S(fir16m_id) fir16e_r=fir16e_results_channel_2->read();
		...
		// send parameters and receive results
		HEPSY_S(fir16m_id) fir16s_parameters_channel_1->write(fir16s_p);
		HEPSY_S(fir16m_id) fir16s_r=fir16s_results_channel_2->read();

		// fill local variables
		HEPSY_S(fir16m_id) for( unsigned j=0; j<TAP16; j++) shift[j]=fir16s_r.shift[j];
		
		// write output values on channel
		HEPSY_S(fir16m_id) result16_channel_1->write(acc);
